        ...
        <form action="{{ route('appointments.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select id="status" name="status" class="form-select" required>
                    <option value="scheduled" {{ old('status', $appointment->status ?? 'available') == 'scheduled' ? 'selected' : '' }}>مجدول</option>
                    <option value="completed" {{ old('status', $appointment->status ?? 'available') == 'completed' ? 'selected' : '' }}>مكتمل</option>
                    <option value="canceled" {{ old('status', $appointment->status ?? 'available') == 'canceled' ? 'selected' : '' }}>ملغي</option>
                    <option value="available" {{ old('status', $appointment->status ?? 'available') == 'available' ? 'selected' : '' }}>متاح للحجز</option>
                </select>
